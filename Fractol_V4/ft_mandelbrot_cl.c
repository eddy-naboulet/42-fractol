...
/*   Updated: 2016/05/02 17:57:40 by enaboule         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cl 		*init_opencl(t_use *use)
{
	...
	cl->sources = get_sources("Fractales/mandelbrot.cl");
	...
	cl->output = clCreateBuffer(cl->context, CL_MEM_WRITE_ONLY, 800 * use->sizeline, NULL, &cl->error);
	cl->global[0] = 800;
	cl->global[1] = 800;
	cl->global[2] = 0;
	return (cl);
...
void 		ft_draw_cl(t_fctl *fctl)
{
	...
	clEnqueueReadBuffer(cl->cmd_queue, cl->output, CL_TRUE, 0, 800 * fctl->use->sizeline,
