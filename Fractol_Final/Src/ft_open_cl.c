...
/*   Updated: 2016/05/07 14:46:27 by enaboule         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cl		*init_opencl(t_use *use, char *select)
{
	...
	cl->properties[2] = 0;
	cl->context = clCreateContext(cl->properties, cl->device_count, &cl->device,
		NULL, NULL, &cl->error);
	cl->sources = get_sources(select);
	cl->cmd_queue = clCreateCommandQueue(cl->context, cl->device, 0,
		&cl->error);
	cl->program = clCreateProgramWithSource(cl->context, 1,
		(const char**)&cl->sources, NULL, &cl->error);
	cl->error = clBuildProgram(cl->program, cl->device_count, &cl->device,
		NULL, NULL, NULL);
	cl->kernel = clCreateKernel(cl->program, "fractal", &cl->error);
	cl->output = clCreateBuffer(cl->context, CL_MEM_WRITE_ONLY, 1024 *
		use->sizeline, NULL, &cl->error);
	cl->global[0] = 1024;
	cl->global[1] = 1024;
	cl->global[2] = 0;
	return (cl);
...
void		ft_draw_cl(t_fctl *fctl, char *select)
{
	...
	clEnqueueNDRangeKernel(cl->cmd_queue, cl->kernel, 2, NULL, cl->global,
		NULL, 0, NULL, NULL);
	clFinish(cl->cmd_queue);
	clEnqueueReadBuffer(cl->cmd_queue, cl->output, CL_TRUE, 0, 1024 *
		fctl->use->sizeline, fctl->use->data, 0, NULL, NULL);
}
